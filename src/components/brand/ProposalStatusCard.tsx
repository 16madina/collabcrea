...
const ProposalStatusCard = ({ offerId, conversationId }: ProposalStatusCardProps) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data: collabData } = await supabase
          .from("collaborations")
          .select("id, status, agreed_amount, creator_id")
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          if (payload.new) {
            const newCollab = payload.new as Collaboration;
  ...
  return (
    ...
    >
      ...
      {statusDisplay.showPayButton && (
        <Button
          variant="gold"
          size="sm"
          className="w-full mt-3"
          onClick={() => navigate("/brand/collabs?tab=collabs")}
        >
